...
function WorkshopDetails() {
  const { id } = useParams();
  const { workshops, user, registerWorkshop, unregisterWorkshop, addFeedback } = useContext(WorkshopContext);
  const { theme } = useContext(ThemeContext);
  const navigate = useNavigate();
  const workshop = workshops.find((w) => w.id === parseInt(id));
  const [isRegistered, setIsRegistered] = useState(user.registeredWorkshops.includes(parseInt(id)));
  ...
  useEffect(() => {
    fetch("/data/feedbacks.json")
      ...
      .then((data) => {
        const workshopFeedbacks = data.filter((f) => f.workshopId === parseInt(id));
        setFeedbacks(workshopFeedbacks);
      })
  ...
  const handleSubmitFeedback = () => {
    if (feedbackText && rating > 0) {
      ...
      addFeedback({ comment: feedbackText, rating });
          ...
          >
            ...
            <div className="flex">
              ...
              <motion.button
                ...
                className={`btn ${theme === "light" ? "light bg-gray-600 hover:bg-gray-700" : "bg-gray-700 hover:bg-gray-800"} ml-2`}
                whileHover={{ scale: 1.03 }}
                whileTap={{ scale: 0.98 }}
              >
                Cancel
